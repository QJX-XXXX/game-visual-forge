...
The provider applies a no-boost -1.0 dBFS peak ceiling before writing generated raw WAV files. Staging processing may attenuate a still-hot non-looping text-to-audio one-shot to the same ceiling, but it must never raise a quiet signal. Skip silent or already clipped staging input so silence remains reviewable and clipping remains a hard-failure signal. Do not apply upward normalization to any mode.
...
Hard failures include decode or format mismatch, wrong duration, empty output, clipping, dense full-duration energy without credible one-shot transient-to-tail decay, missing previews, stale hashes, failed protected-region comparison, and missing loop evidence when looping is requested. Speech/music detection is advisory only; the user must still complete the semantic listening check.
