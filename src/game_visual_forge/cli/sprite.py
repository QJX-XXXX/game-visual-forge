...
from game_visual_forge.cli.planning import build_sprite_execution_plan
...
from game_visual_forge.processing.sprite import ProcessingResult, process_sprite, publish_verified_outputs
from game_visual_forge.quality import apply_visual_review, build_asset_manifest, validate_sprite_outputs
from game_visual_forge.routing import AgentImageCapabilities, NativeAttemptOutcome, route_sprite


def _request(path: Path) -> tuple[SpriteRequest, str]:
    request = SpriteRequest.from_dict(load_json(path))
    return request, fingerprint_request(request.to_dict())


def run_sprite_plan(request_path: Path, out_dir: Path, now: str) -> dict[str, Any]:
    request, fingerprint = _request(request_path)
    out_dir = out_dir.resolve()
    normalized = out_dir / "sprite-request.json"
    plan_path = out_dir / "execution-plan.json"
    state_path = out_dir / "job-state.json"
    state = JobState(1, f"job-{request.asset_id}", request.asset_id, JobStatus.PLANNED, now, now, fingerprint)
    dump_json(normalized, request.to_dict())
    dump_json(plan_path, build_sprite_execution_plan(request).to_dict())
    save_job(state_path, state)
    return {"schema_version": 1, "status": state.status.value, "request_path": "sprite-request.json", "plan_path": "execution-plan.json", "state_path": "job-state.json"}


def run_sprite_route(
    request_path: Path,
    ...
    capabilities = AgentImageCapabilities(bool(capabilities_payload["supported"]), tuple(capabilities_payload["operations"]))
    preflight = None if preflight_path is None else ProviderPreflight.from_dict(load_json(preflight_path))
    decision = route_sprite(
        request,
        capabilities,
        native_outcome=NativeAttemptOutcome(native_outcome),
        selected_source=None if selection is None else SourceType(selection),
        provider_preflight=preflight,
    )
    dump_json(out_path, decision.to_dict())
    state = load_job(state_path)
    if state.request_fingerprint != fingerprint:
        raise ValueError("request fingerprint does not match job state")
    if not decision.requires_user_selection:
        ...
        save_job(state_path, state)
    return {"schema_version": 1, "status": state.status.value, "decision_path": str(out_path.name), "source_type": decision.source_type.value if decision.source_type else None, "requires_user_selection": decision.requires_user_selection, "requires_paid_confirmation": decision.requires_paid_confirmation}


def run_sprite_ingest(
    request_path: Path,
    decision_path: Path,
    image_path: Path,
    repo_root: Path,
    ...
    request, fingerprint = _request(request_path)
    decision = SourceDecision.from_dict(load_json(decision_path))
    if decision.request_fingerprint != fingerprint or decision.source_type is None:
        raise ValueError("source decision does not match request or has no source")
    record = ingest_image(repo_root, image_path, decision.source_type, fingerprint, provider=decision.selected_provider)
...
def run_sprite_process(
    ...
    if record.request_fingerprint != fingerprint:
        raise ValueError("raw image fingerprint does not match request")
    result = process_sprite(repo_root, request, record, repo_root / PurePosixPath(request.output_dir))
...
def run_sprite_validate(
    ...
    request, fingerprint = _request(request_path)
    record = RawImageRecord.from_dict(load_json(raw_image_path))
    result = ProcessingResult.from_dict(load_json(processing_result_path))
    if record.request_fingerprint != fingerprint:
        raise ValueError("raw image fingerprint does not match request")
    report = validate_sprite_outputs(staging_dir, request, record, result)
    if visual_review_path is not None:
        report = apply_visual_review(report, load_json(visual_review_path))
    manifest = build_asset_manifest(staging_dir, request, record, result, report)
    ...
    if report.deterministic_status is not QualityStatus.FAILED:
        target = JobStatus.COMPLETED if published else JobStatus.NEEDS_ATTENTION
    else:
